...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      themeMode: themeMode,
      // Keep the status bar icons readable on both backgrounds, including
      // on screens that have no AppBar.
      builder: (context, child) {
        final isDark = Theme.of(context).brightness == Brightness.dark;
        return AnnotatedRegion<SystemUiOverlayStyle>(
          value: (isDark ? SystemUiOverlayStyle.light : SystemUiOverlayStyle.dark).copyWith(statusBarColor: Colors.transparent),
          child: child!,
        );
      },
      // Shows the login page when signed out, the app when signed in.
      home: const AuthGate(),
    );
  }
}
